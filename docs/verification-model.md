...
Before reading diff evidence, every selection requires a resolved Git index without `assume-unchanged` or `skip-worktree` entries. A bounded NUL-delimited index inventory checks flags and merge stages independently of `git diff`'s cached filesystem assumptions. Hidden entries (including sparse checkout) and unresolved merges abort analysis before repository execution; they cannot yield a successful empty-change report. This conservative precondition applies even if a flagged file currently appears unchanged. It does not mutate the index and does not provide concurrent filesystem isolation.
...
- Gitlink changes are visible as metadata-only `unknown` assessments with optional `submodule: true`; either side being mode `160000` makes a type transition conservative too. Zero line counts mean no source-line measurement, not absence of nested changes. Nested contents, nested dirty files, and nested test results are not analyzed or verified. Test maps and LCOV cannot upgrade gitlink evidence. Static immutable analysis remains available, but immutable check execution rejects indexed submodules because their filesystem inputs are not snapshot-bound. Working-tree execution is explicitly weaker and retains unknown gitlink evidence.
