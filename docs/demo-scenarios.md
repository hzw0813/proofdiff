...
| --- | --- | --- | --- |
| Mixed evidence | JavaScript discount validation plus a Python retry loop | One related test-file pass (`verified` in JSON), one unverified file; overall partial | Demonstrates transitive dependency impact, targeted related-test-file success, and honest missing Python evidence. |
...
The gallery landing hero is generated from the opaque-test-script result. It reads the real passing check, `src/access.js` assessment, `test/access.test.js` relationship, and zero-related-test-file-pass/one-partial counts from that report. Generation fails if the scenario stops matching those facts, so the first-screen contrast cannot silently drift into a hand-authored claim. The headline asks which related test files actually ran; ProofDiff does not claim that changed symbols or lines ran.
