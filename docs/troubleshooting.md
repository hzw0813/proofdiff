...
ProofDiff deliberately supports only bounded, high-evidence metadata shapes. Compiler mappings require the nearest ancestor `tsconfig.json`, repository-relative string inheritance, and an exact or single-wildcard `paths` key; `baseUrl` only anchors those targets. Package imports must use the nearest owning package's exact declared name and exact export key. Export patterns/arrays, unknown runtime-condition choices, package-based configuration inheritance, standalone `baseUrl`, workspace dependency linking, third-party packages, and `node_modules` remain unresolved. Malformed, cyclic, excessive, ambiguous, escaping, or symlinked metadata fails closed and may appear in report notes.
