...
All diff modes also require a resolved index without `assume-unchanged` or `skip-worktree` entries. These flags can hide modified or missing source, configuration, and tests from Git while ProofDiff reads the filesystem. Unresolved merges and sparse checkouts therefore stop with an actionable error (CLI exit 2, even with `--fail-on never`) before checks run or a report is written. Resolve and stage conflicts, use a full checkout (`git sparse-checkout disable`), or clear manually set flags with `git update-index --no-assume-unchanged --no-skip-worktree -- <path>`. ProofDiff never clears flags or expands the checkout itself.

Submodule pointer additions, updates, renames, deletions, and file-type transitions are visible as **unknown** metadata-only changes. They carry `submodule: true` in JSON, with zero source-line counts and no source hunks. Nested contents and content-only dirty state are outside the report; even passing superproject checks, test-map declarations, or LCOV cannot verify a submodule. Analyze each nested repository separately. Python check discovery does not enter submodules or embedded Git repositories. Static immutable analysis is supported, but `--run-checks` with `--base`, `--range`, or `--staged` rejects a checkout containing submodules because nested execution inputs are not snapshot-bound. Explicit working-tree check execution remains available and keeps submodule evidence unknown.
