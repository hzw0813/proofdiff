...
function checkApplies(check: CheckResult, file: ChangedFile, relatedTests: string[]): boolean {
  ...
  if (check.id.startsWith("js:")) return file.language === "javascript" || file.language === "typescript";
  if (check.id.startsWith("python:")) return file.language === "python";
  return true;
}
...
function verificationFor(file: ChangedFile, relatedTests: string[], checks: CheckResult[], declaredTests: string[]): { status: VerificationStatus; evidence: EvidenceItem[]; executedTests: string[]; testExecutions: FileAssessment["testExecutions"] } {
  ...
  if (relatedTests.length > 0) {
    ...
    if (inferredCount > 0) relationshipParts.push(`${inferredCount} from resolved local dependency paths or accepted runner qualification`);
    if (declaredTests.length > 0) relationshipParts.push(`${declaredTests.length} user-declared by --test-map`);
    evidence.push({
      kind: "related-test",
  ...
  if (targetedFailures.length > 0 || unlocalizedTargetedFailures.length > 0 || opaqueFailures.length > 0 || operationalFailures.length > 0) return { status: "verification-failed", evidence, executedTests, testExecutions };
  ...
  if (executedTests.length > 0) return { status: "verified", evidence, executedTests, testExecutions };
  if (passing.length > 0) return { status: "partially-verified", evidence, executedTests, testExecutions };
  return { status: "unverified", evidence, executedTests, testExecutions };
}
...
function riskFor(file: ChangedFile, status: VerificationStatus, relatedTests: string[], impacted: string[], analysis: SourceAnalysis | undefined): { score: number; level: RiskLevel; reasons: string[] } {
  let score = 10;
  ...
  if (analysis?.confidence === "low") { score += 12; reasons.push("Structural analysis used a low-confidence fallback."); }
  if (analysis && analysis.diagnostics.length > 0) { score += 6; reasons.push("The parser reported diagnostics."); }
  if (changedLines >= 300) { score += 18; reasons.push(`Large change surface (${changedLines} changed lines).`); }
  else if (changedLines >= 100) { score += 9; reasons.push(`Broad change surface (${changedLines} changed lines).`); }
  if (impacted.length >= 25) { score += 16; reasons.push(`May affect at least ${impacted.length} dependent files.`); }
  else if (impacted.length >= 5) { score += 8; reasons.push(`May affect ${impacted.length} dependent files.`); }
  if (/^(?:package(?:-lock)?\.json|pnpm-lock\.yaml|yarn\.lock|pyproject\.toml|requirements[^/]*\.txt)$/.test(path.basename(file.path))) {
    score += 15; reasons.push("Dependency or build metadata changed.");
  }
  if (/(?:^|\/)(?:access|auth|crypto|security|permissions?|payments?)(?:\/|\.|$)/i.test(file.path)) {
    score += 15; reasons.push("Security-sensitive path name warrants focused human review.");
  }
  const bounded = clamp(score, 0, 100);
  const level: RiskLevel = bounded >= 80 ? "critical" : bounded >= 55 ? "high" : bounded >= 30 ? "medium" : "low";
