...
type FailPolicy = "never" | "failed" | "unverified" | "partial" | "high-risk";
...
function parseArgs(args: string[]): CliOptions | "help" | "version" {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--run-checks") { options.runChecks = true; continue; }
    ...
    if (arg === "--repo") { options.repo = valueAfter(args, index, arg); index += 1; continue; }
    if (arg === "--base") { options.base = valueAfter(args, index, arg); index += 1; continue; }
    ...
    if (arg === "--github-summary") { options.githubSummary = valueAfter(args, index, arg); index += 1; continue; }
    if (arg === "--timeout") {
      const seconds = Number(valueAfter(args, index, arg));
      if (!Number.isFinite(seconds) || seconds < 1 || seconds > 1_800) throw new UsageError("--timeout must be between 1 and 1800 seconds.");
      options.timeoutMs = Math.round(seconds * 1_000); index += 1; continue;
    }
    if (arg === "--format") {
      const value = valueAfter(args, index, arg);
      if (value !== "terminal" && value !== "json") throw new UsageError("--format must be terminal or json.");
      options.format = value; index += 1; continue;
    }
    if (arg === "--fail-on") {
      const value = valueAfter(args, index, arg) as FailPolicy;
      if (!["never", "failed", "unverified", "partial", "high-risk"].includes(value)) throw new UsageError("--fail-on must be never, failed, unverified, partial, or high-risk.");
      options.failOn = value; index += 1; continue;
    }
    throw new UsageError(`Unknown option: ${arg}`);
  }
  if (options.selectedChecks.length > 0 && !options.runChecks) throw new UsageError("--check requires --run-checks; static analysis does not execute repository code.");
...
function policyFailed(report: AnalysisReport, policy: FailPolicy): boolean {
  if (policy === "never") return false;
  if (policy === "failed") return report.summary.overallStatus === "verification-failed";
  if (policy === "unverified") return report.assessments.some((item) => item.status === "verification-failed" || item.status === "unverified");
  if (policy === "partial") return report.assessments.some((item) => item.status !== "verified");
...
async function main(): Promise<void> {
  ...
  if (parsed.output) await writeOutput(parsed.output, primary);
  else process.stdout.write(primary.endsWith("\n") ? primary : `${primary}\n`);
  if (parsed.html) {
    await writeOutput(parsed.html, renderHtmlReport(report));
    if (parsed.format === "terminal") process.stdout.write(`HTML report: ${path.resolve(parsed.html)}\n`);
  }
  ...
  if (policyFailed(report, parsed.failOn)) process.exitCode = 1;
}
