...
async function detectPythonTests(root: string, limit = 2_000): Promise<PythonTestLayout | null> {
  ...
  while (queue.length > 0 && inspected < limit) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory() && !entry.isSymbolicLink() && !["node_modules", ".git", "__pycache__"].includes(entry.name)) queue.push({ absolute: target, directory: current.directory });
...
function targetingForScript(kind: CheckDefinition["kind"], command: string): Pick<CheckDefinition, "targetRunner" | "targetPattern" | "targetPatterns"> | null {
  ...
  if (arguments_.every((argument) => /^[A-Za-z0-9_./-]+\.[cm]?js$/.test(argument))) return { targetRunner: "node-test", targetPatterns: arguments_ };
  return null;
}
...
function compiledNodeTarget(file: string, pattern: string | undefined, patterns: string[] | undefined): string | null {
  if (patterns?.length) {
    if (/\.[cm]?jsx?$/.test(file)) return patterns.includes(file) ? file : null;
  ...
  if (pattern === undefined) return /\.[cm]?jsx?$/.test(file) ? file : null;
  if (!/\.(?:ts|tsx|mts|cts)$/.test(file)) return null;
  ...
  if (!compiledName.startsWith(before) || !compiledName.endsWith(after)) return null;
  return path.posix.join(path.posix.dirname(pattern), compiledName);
}

export async function targetedTestChecks(root: string, definitions: CheckDefinition[], relatedTests: string[], limit = 100): Promise<{ checks: CheckDefinition[]; truncated: boolean }> {
  const sorted = unique(relatedTests).sort();
  const checks: CheckDefinition[] = [];
  let truncated = false;
  for (const definition of definitions) {
    if (!definition.targetRunner) continue;
    const candidates: Array<{ source: string; argument: string }> = [];
    for (const file of sorted) {
      if (definition.targetRunner === "node-test") {
        ...
        if (argument && await pathExists(path.join(root, argument))) candidates.push({ source: file, argument });
