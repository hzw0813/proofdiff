...
Non-relative edges require explicit repository metadata. For compiler paths, the nearest ancestor `tsconfig.json` may inherit repository-relative string configurations; exact keys win and otherwise the longest-prefix single-wildcard key supplies ordered fallback targets. `baseUrl` is used only to anchor `paths`, not as a standalone resolver. For package self-references, the nearest package boundary must declare the exact package name and export subpath; only direct targets or safely selected explicit source/default conditions are accepted. Package-export patterns, arrays, unmodeled runtime conditions, third-party packages, and `node_modules` remain unresolved. Successful edges retain their specifier, mechanism, metadata source, matched key, target, confidence, and static-only limitation inside the graph.
