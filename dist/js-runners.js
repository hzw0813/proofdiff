...
function safeEnvironmentAssignment(token) {
    const separator = token.indexOf("=");
    if (separator <= 0)
        return null;
    const name = token.slice(0, separator);
    const value = token.slice(separator + 1);
    if (!ENV_NAME.test(name) || BLOCKED_ENV_NAMES.has(name) || !ENV_LITERAL_VALUE.test(value))
        return null;
    return [name, value];
}
function recognizedRunnerScript(command) {
    ...
    const runnerEnv = Object.create(null);
    ...
    while (index < tokens.length) {
        ...
        if (environmentAssignments > MAX_RUNNER_ENV_ASSIGNMENTS)
            return null;
        const [name, value] = assignment;
        if (Object.hasOwn(runnerEnv, name))
            return null;
        runnerEnv[name] = value;
        index += 1;
    }
    if (usesCrossEnv && environmentAssignments === 0)
        return null;
...
async function localCrossEnvAvailable(root) {
    return pathExists(path.join(root, "node_modules", "cross-env", "package.json"));
}
function qualifyTarget(runner, inputFile) {
    const file = normalizedRepositoryPath(inputFile);
    if (!JS_TEST_EXTENSION.test(file) || !isTestLikePath(file))
        return null;
    return {
        path: inputFile,
        runnerPath: file,
...
function observerSource(runner, runnerBin, runnerArgs, runnerEnv, targets) {
    return String.raw `
...
export async function targetedJsFrameworkChecks(root, definitions, impactedPaths, limit = 100) {
    ...
    for (const definition of definitions) {
        if (definition.kind !== "test" || !definition.id.startsWith("js:test:"))
            continue;
        ...
        if (recognized === null)
            continue;
        if (recognized.usesCrossEnv && !(await localCrossEnvAvailable(root)))
            continue;
...
//# sourceMappingURL=js-runners.js.map