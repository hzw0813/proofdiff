...
function assessmentCard(item) {
    ...
    const executed = item.testExecutions.length
        ...
        : `<div><h4>Targeted test outcomes</h4><p class="muted">None observed. Related-file presence alone cannot produce “Verified.”</p></div>`;
...
export function renderHtmlReport(report) {
    const countTiles = ["verified", "partially-verified", "unverified", "unknown", "verification-failed"]
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 15% -10%,#123140 0,transparent 35%),radial-gradient(circle at 90% 0,#251d43 0,transparent 32%),var(--bg);color:var(--text);font:15px/1.55 ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;min-height:100vh}
    ...
    @media(max-width:520px){main{width:min(100% - 24px,1180px)}h1{font-size:42px}.overview{padding:18px}.overall{align-items:flex-start}.metrics{grid-template-columns:repeat(2,1fr)}.trust{flex-direction:column;gap:5px}.trust b{white-space:normal}.controls{display:grid;grid-template-columns:1fr 1fr}.controls input{grid-column:1/-1}.file-card{padding:16px}.file-badges,.symbols,.file-card>details>summary{margin-left:0}.status,.risk{font-size:10px}}
...
<main>
  <header>
    ...
    <h1>What shows this change <em>is safe?</em></h1>
...
${noteMarkup ? `  ${noteMarkup}\n` : ""}  <p class="method"><strong>How to read this:</strong> “Verified” means ProofDiff passed a statically related test file directly to a recognized runner and observed success. It is test-file execution evidence—not changed-line coverage, mathematical proof, or a guarantee of safety. Inference and limitations are labeled separately.</p>
