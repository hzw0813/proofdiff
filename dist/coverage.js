...
function sourceToRepoPath(root, source) {
...
async function parseLcov(root, file) {
    ...
    if (!metadata.isFile())
        throw new CoverageError(`Coverage artifact is not a file: ${file}`);
    if (metadata.size > MAX_COVERAGE_BYTES)
        throw new CoverageError(`Coverage artifact exceeds the ${MAX_COVERAGE_BYTES / (1024 * 1024)} MB limit.`);
    ...
    for (const record of raw.split(/\r?\n/)) {
        if (record.startsWith("SF:")) {
            ...
            if (current !== null && !files.has(current)) {
                if (files.size >= MAX_COVERAGE_FILES)
                    throw new CoverageError(`Coverage artifact exceeds the ${MAX_COVERAGE_FILES} file limit.`);
        ...
        if (record.startsWith("DA:")) {
            ...
            if (!match || match[1] === undefined || match[2] === undefined)
                throw new CoverageError("Coverage artifact contains a malformed DA record.");
            if (current === undefined)
                throw new CoverageError("Coverage artifact contains a DA record before any SF record.");
            const line = Number(match[1]);
            const count = Number(match[2]);
            if (!Number.isSafeInteger(line) || line < 1 || !Number.isSafeInteger(count) || count < 0)
                throw new CoverageError("Coverage artifact contains an invalid line or hit count.");
            lineRecords += 1;
            if (lineRecords > MAX_COVERAGE_LINES)
                throw new CoverageError(`Coverage artifact exceeds the ${MAX_COVERAGE_LINES} line-record limit.`);
            if (current !== null) {
...
function changedCurrentLines(file) {
    if (file.binary || file.change === "deleted" || file.additions === 0)
        return { lines: [], exact: true };
    const lines = new Set();
    for (const hunk of file.hunks) {
        for (let line = hunk.newRange.start; line <= hunk.newRange.end; line += 1)
            lines.add(line);
    }
    const sorted = [...lines].sort((a, b) => a - b);
    return { lines: sorted, exact: sorted.length === file.additions };
}
function evaluateFile(file, coverage) {
    ...
    const state = covered.length === changed.lines.length
        ? "all-covered"
        : covered.length > 0
            ? "partially-covered"
            : measured.length > 0
...
export async function loadCoverageEvidence(root, selection, coverageFile, coverageCommit, changedFiles) {
    ...
    for (const file of changedFiles)
        byPath.set(file.path, evaluateFile(file, parsed.files.get(file.path)));
    return {
        summary: {
            format: "lcov",
            artifact,
...
export function attachCoverageEvidence(item, coverage) {
    if (!coverage)
        return item;
