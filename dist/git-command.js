...
async function rawGit(root, args, overrides, options) {
    ...
    if (result.error !== undefined)
        throw new GitError(`Could not run Git: ${result.error}`);
    if (result.timedOut)
        throw new GitError("Git inspection timed out; incomplete output cannot establish evidence.");
    if (result.truncated)
        throw new GitError(`Git output exceeded the ${maxOutputBytes} byte limit; incomplete output cannot establish evidence. Narrow the selected diff or use a smaller checkout.`);
    return result;
}
...
export async function runGit(root, args, options = {}) {
    ...
    for (const prefix of prefixes) {
        ...
        for (const property of properties)
            overrides.push("-c", `${prefix}.${property}=${property === "required" ? "false" : ""}`);
    }
