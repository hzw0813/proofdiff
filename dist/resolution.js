...
export function javascriptModuleCandidates(baseInput) {
    const base = normalizeRepoPath(path.posix.normalize(baseInput));
    if (!isRepositoryPath(base))
        return [];
...
function explicitConfiguredModuleCandidates(base) {
    if (base.endsWith(".js")) {
    ...
    if ([".ts", ".tsx", ".mts", ".cts"].some((extension) => base.endsWith(extension)))
        return [base];
    return null;
}
function extensionlessConfiguredModuleCandidates(base) {
...
export class BoundedStaticModuleResolver {
    ...
    loadedConfigs = 0;
    loadedPackages = 0;
    nonRelativeImports = 0;
    diagnosticCount = 0;
    diagnosticLimitReported = false;
    constructor(root, repositoryFiles, available, diagnostics) {
        this.root = root;
    ...
    async resolve(importer, specifier) {
        ...
        if (compilerAttempt.kind === "resolved")
            return this.record(compilerAttempt.resolution);
    ...
    async readMetadata(metadataPath, label) {
    ...
    async targetIsInside(target, directory = "") {
    ...
    async hiddenPrecedenceCandidate(candidates) {
        for (const candidate of candidates) {
            if (!this.available.has(candidate) && await this.hiddenFileExists(candidate))
                return candidate;
        }
        return null;
    }
    async nearestMetadataPath(importer, filename) {
        ...
        if (cache.has(importerDirectory))
            return cache.get(importerDirectory) ?? null;
    ...
    async loadConfig(configPath, stack) {
        ...
        if (options && Object.hasOwn(options, "baseUrl")) {
            ...
            if (options.baseUrl.includes("${"))
                return this.rejectConfig(configPath, "baseUrl template variables are unsupported");
        ...
        if (options && Object.hasOwn(options, "declarationDir")) {
            if (typeof options.declarationDir !== "string" || options.declarationDir.includes("${"))
                return this.rejectConfig(configPath, "declarationDir must be a repository-relative string");
            const resolved = joinRepositoryPath(path.posix.dirname(configPath), options.declarationDir);
            if (resolved === null)
                return this.rejectConfig(configPath, "declarationDir must remain inside the repository");
        ...
        if (options && Object.hasOwn(options, "customConditions")) {
            ...
            config.customConditions = unique(options.customConditions);
        }
        if (options && Object.hasOwn(options, "moduleResolution")) {
            if (typeof options.moduleResolution !== "string")
                return this.rejectConfig(configPath, "moduleResolution must be a string");
        ...
        if (options && Object.hasOwn(options, "resolvePackageJsonExports")) {
            if (typeof options.resolvePackageJsonExports !== "boolean")
                return this.rejectConfig(configPath, "resolvePackageJsonExports must be boolean");
            config.resolvePackageJsonExports = options.resolvePackageJsonExports;
        }
        this.configCache.set(configPath, config);
        return config;
    }
    rejectConfig(configPath, reason) {
    ...
    resolveExtends(configPath, value) {
        ...
        if (joined === null || joined.split("/").includes("node_modules") || joined.includes("${"))
            return null;
    ...
    parsePaths(configPath, value) {
        if (!isRecord(value))
            return this.rejectConfig(configPath, "paths must be an object");
        const rawEntries = Object.entries(value);
        if (rawEntries.length > MAX_PATH_MAPPINGS)
            return this.rejectConfig(configPath, `paths exceeds ${MAX_PATH_MAPPINGS} mappings`);
        const entries = new Map();
        for (const [key, targets] of rawEntries) {
            const keyWildcards = key.split("*").length - 1;
            if (key.length === 0 || key.startsWith(".") || keyWildcards > 1)
                return this.rejectConfig(configPath, `unsupported paths key ${JSON.stringify(key)}`);
            ...
            for (const target of targets) {
                ...
                if (target.includes("${") || targetWildcards > 1 || (targetWildcards === 1 && keyWildcards === 0))
                    return this.rejectConfig(configPath, `unsupported wildcard or template target for ${JSON.stringify(key)}`);
            }
            entries.set(key, [...targets]);
        }
        return { entries, origin: configPath };
    }
    matchCompilerPath(paths, specifier) {
        const exact = paths.entries.get(specifier);
        if (exact)
            return { key: specifier, capture: null, targets: exact };
        const matches = [];
    ...
    async resolveCompilerPath(importer, specifier) {
        ...
        const attemptCandidates = async (candidates) => {
            ...
            if (candidateCount > MAX_CANDIDATES_PER_IMPORT)
                return { kind: "blocked", reason: `candidate expansion exceeded ${MAX_CANDIDATES_PER_IMPORT}` };
            ...
            if (hidden)
                return { kind: "blocked", reason: `higher-precedence candidate ${hidden} exists outside the bounded Git inventory` };
            if (!(await this.targetIsInside(target)))
                return { kind: "blocked", reason: `mapped target ${target} resolves outside the repository through a symlink` };
            return { kind: "resolved", target };
        };
        const resolvedAttempt = (target, lookup) => ({
            kind: "resolved",
            resolution: {
                target,
                evidence: {
        ...
        for (const targetPattern of match.targets) {
            ...
            if (mapped === null)
                return { kind: "blocked", reason: `mapped target ${JSON.stringify(substituted)} escapes the repository` };
            if (mapped.split("/").includes("node_modules"))
                return { kind: "blocked", reason: "mapped target enters node_modules, which is outside the supported repository-local subset" };
            if (path.posix.extname(mapped) !== "") {
                const candidates = explicitConfiguredModuleCandidates(mapped);
                if (candidates === null)
                    return { kind: "blocked", reason: `mapped target ${mapped} has an unsupported explicit extension` };
                const attempt = await attemptCandidates(candidates);
                if (attempt.kind === "blocked")
                    return attempt;
                if (attempt.kind === "resolved")
                    return resolvedAttempt(attempt.target, "documented explicit-extension substitution");
                continue;
            }
            ...
            if (fileAttempt.kind === "blocked")
                return fileAttempt;
            if (fileAttempt.kind === "resolved")
                return resolvedAttempt(fileAttempt.target, `${config.moduleResolution} extensionless file lookup`);
            ...
            if (indexAttempt.kind === "blocked")
                return indexAttempt;
            if (indexAttempt.kind === "resolved")
                return resolvedAttempt(indexAttempt.target, `${config.moduleResolution} directory index lookup`);
            for (const extension of OMITTED_EXPLICIT_EXTENSIONS) {
    ...
    async loadPackage(packagePath) {
        if (this.packageCache.has(packagePath))
            return this.packageCache.get(packagePath) ?? null;
    ...
    exportEntry(exportsValue, exportKey) {
        if (!isRecord(exportsValue))
            return exportKey === "." ? { value: exportsValue, matchedKey: "." } : null;
        ...
        if (Object.hasOwn(exportsValue, exportKey))
            return { value: exportsValue[exportKey], matchedKey: exportKey };
        if (subpathKeys.some((key) => key.includes("*")))
            return "unsupported";
        return null;
    }
    selectConditionTarget(value, activeConditions, state, depth = 0) {
        if (typeof value === "string")
            return { target: value, conditions: [] };
        if (!isRecord(value) || depth >= MAX_EXPORT_CONDITION_DEPTH)
            return "unsupported";
        ...
        for (const condition of keys) {
            state.branches += 1;
            if (state.branches > MAX_EXPORT_BRANCHES)
                return "unsupported";
            if (condition === "default" || activeConditions.has(condition)) {
                ...
                if (selected === "unsupported" || selected === null)
                    return selected;
    ...
    async resolvePackageSelfReference(importer, specifier) {
        ...
        if (!metadata?.name || metadata.exports === undefined)
            return null;
        if (specifier !== metadata.name && !specifier.startsWith(`${metadata.name}/`))
            return null;
