...
function exactKeys(value, allowed, label) {
    ...
    if (extra.length > 0)
        throw new TestMapError(`${label} contains unsupported field${extra.length === 1 ? "" : "s"}: ${extra.join(", ")}.`);
}
export async function testMapRepositoryPath(root, file) {
...
function normalizeDeclaredPath(root, value, label) {
    if (typeof value !== "string" || value.length === 0)
        throw new TestMapError(`${label} must be a non-empty repository-relative path.`);
...
export async function loadTestMap(root, file, repositoryFiles, selectedSourcePaths = []) {
    ...
    if (metadata.isSymbolicLink())
        throw new TestMapError(`Test map must not be a symbolic link: ${file}`);
    if (!metadata.isFile())
        throw new TestMapError(`Test map is not a file: ${file}`);
    if (metadata.size > MAX_TEST_MAP_BYTES)
        throw new TestMapError(`Test map exceeds the ${MAX_TEST_MAP_BYTES / 1024} KB limit.`);
    let parsed;
    try {
    ...
    if (!plainObject(parsed))
        throw new TestMapError("Test map root must be an object.");
    exactKeys(parsed, ["version", "relationships"], "Test map root");
    if (parsed.version !== 1)
        throw new TestMapError("Test map version must be 1.");
    if (!Array.isArray(parsed.relationships))
        throw new TestMapError("Test map relationships must be an array.");
    if (parsed.relationships.length > MAX_RELATIONSHIPS)
        throw new TestMapError(`Test map exceeds the ${MAX_RELATIONSHIPS} relationship limit.`);
    ...
    for (let index = 0; index < parsed.relationships.length; index += 1) {
        ...
        if (!plainObject(relationship))
            throw new TestMapError(`${label} must be an object.`);
        exactKeys(relationship, ["source", "tests"], label);
        const source = normalizeDeclaredPath(root, relationship.source, `${label}.source`);
        if (!selectableSources.has(source))
            throw new TestMapError(`${label} references a source path that is neither Git-visible nor selected as changed: ${source}`);
        if (bySource.has(source))
            throw new TestMapError(`Test map declares source more than once: ${source}`);
        if (!Array.isArray(relationship.tests) || relationship.tests.length === 0)
            throw new TestMapError(`${label}.tests must be a non-empty array.`);
        if (relationship.tests.length > MAX_TESTS_PER_RELATIONSHIP)
            throw new TestMapError(`${label}.tests exceeds the ${MAX_TESTS_PER_RELATIONSHIP} path limit.`);
        const tests = [];
        const seen = new Set();
        for (let testIndex = 0; testIndex < relationship.tests.length; testIndex += 1) {
            ...
            if (testPath === source)
                throw new TestMapError(`${label} cannot declare a source as its own related test.`);
            if (seen.has(testPath))
                throw new TestMapError(`${label} declares the same test path more than once: ${testPath}`);
            if (!visible.has(testPath))
                throw new TestMapError(`${label} references a test path that is not Git-visible in the repository: ${testPath}`);
            ...
            tests.push(testPath);
            testPaths += 1;
            if (testPaths > MAX_TOTAL_TESTS)
                throw new TestMapError(`Test map exceeds the ${MAX_TOTAL_TESTS} total test-path limit.`);
        }
