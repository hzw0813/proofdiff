...
All static Git reads share the same hardened launcher, including the filesystem-alignment check for immutable selections and test-map snapshot reads. Content-driver names are read as bounded NUL-delimited data from the effective repository/worktree configuration before each operation; the configuration is not cached across library calls. Incomplete configuration discovery or Git output aborts analysis. Per-file diff paths use literal pathspecs, so filename wildcards cannot change the scope of evidence. These measures retain the separate trusted-Git-directory boundary above; they do not protect against another process concurrently replacing repository/configuration files during analysis.
...
Submodule inspection observes gitlink pointer changes only. It does not initialize, update, fetch, or recursively analyze nested repositories. ProofDiff's static source and Python test discovery exclude nested repositories, and Git's nested patch format is forced to short form. Content-only dirtiness inside a submodule is outside the reported dirty status and evidence scope. Immutable `--run-checks` selections reject indexed submodules before execution because arbitrary root commands can consume nested inputs that ProofDiff has not snapshot-bound. Working-tree execution retains explicit consent and arbitrary-code risks: root commands may still read or execute nested code, but their results cannot verify submodule changes.
